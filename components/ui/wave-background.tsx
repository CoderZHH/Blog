...
export function Waves({
  ...
  const updateMousePosition = (x: number, y: number) => {
    ...
    boundingRef.current = nextBounds

    const mouse = mouseRef.current
    mouse.x = x - nextBounds.left
    mouse.y = y - nextBounds.top
  ...
  const movePoints = (time: number) => {
    ...
    lines.forEach((points) => {
      points.forEach((p: Point) => {
        ...
        const velocity = Math.max(12, mouse.vs)
        const l = Math.max(175, velocity)

        if (d < l) {
          const s = 1 - d / l
          const f = Math.cos(d * 0.001) * s

          p.cursor.vx += Math.cos(mouse.a) * f * l * velocity * 0.00035
          p.cursor.vy += Math.sin(mouse.a) * f * l * velocity * 0.00035
        }
